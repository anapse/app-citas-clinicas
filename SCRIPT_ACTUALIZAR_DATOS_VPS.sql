-- Script completo para actualizar datos según especificaciones del jefe
...
-- Limpiar tablas usando DELETE (más seguro que TRUNCATE con foreign keys)
...
-- Crear tabla Doctor_Especialidades si no existe (para relación muchos a muchos)
...
-- Agregar las nuevas columnas que faltan en la tabla Especialidades
...
BEGIN
    ...
    ALTER TABLE Horarios_Doctores ADD FOREIGN KEY (especialidad_id) REFERENCES Especialidades(id);
END
...
-- Actualizar las especialidades con las nuevas columnas después de crearlas
